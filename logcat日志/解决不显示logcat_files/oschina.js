...
function unfollow(uid) {
	ajax_post("/action/ajax/unfollow?id="+uid,"",function(data){$('#LinkFollow').html(data);});
}

function just_unfollow(uid,f_callback) {
	ajax_post("/action/ajax/unfollow?id="+uid,"",function(data){
		if(f_callback)
			f_callback(null)
	});	
}
