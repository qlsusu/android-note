...
if (typeof global_html_pool == 'undefined') {
	var global_html_pool = [];
}

if (typeof global_script_pool == 'undefined') {
    var global_script_pool = [];
}

if (typeof document_buffer == 'undefined') {
    var document_buffer = {};
}
...
var hint_num = 10;
var post_num = 10;
var rel_script_src = 'http://rel.discuz.soso.com/hu?version=13&site_id=' + my_siteid + '&thread_id=' + rel_tid + '&user_id=' + rel_uid + '&num=' + post_num + '&ti=' + rel_title + '&hint_num=' + hint_num + '&cs=' + charset;
// Relate subject CSS
var rsCss = [
	{
		'selector': '.rs_main',
		'style': 'background: #FFF; font: 12px/1.5 Tahoma, Helvetica, \'Simsun\', sans-serif; color: #444; padding: 9px 10px 10px; min-width: 735px; border: 1px solid #94CFDD; background: #EEF7FC;'
	},
	{
		'selector': '.rs_head',
		'style': 'overflow: hidden; zoom: 1; margin-bottom: 5px; padding-left: 95px;'
	},
	{
		'selector': '.rs_head h3',
		'style': 'float: left; display: inline; margin-left: -95px; width: 75px; font-size: 1em; background: transparent'
	},
	{
		'selector': '.rs_head h3 .has_adv',
		'style': 'padding-left: 20px; background: url(' + rel_resource_url + '/soso_icon.png) no-repeat 0 50%;'
	},
	{
		'selector': '.show_more',
	...
	{
		'selector': '.show_more a',
		'style': 'color: rgb(51, 102, 153);'
	},
	{
		'selector': '.kws_wrapper',
		'style': 'padding: 0 5px; overflow: hidden; zoom: 1; width: 725px;'
	},
	{
		'selector': '.kws_wrapper a',
		'style': 'display: block-inline; float: left; width: 135px; height: 16px; overflow: hidden; color: rgb(51, 102, 153); text-decoration: none;'
	},
	{
		'selector': '#rel_list',
		'style': 'overflow: hidden; zoom: 1; width: 735px; padding: 0; margin: 0;'
	},
	{
		'selector': '#rel_list li',
		'style': 'float: left !important; overflow: hidden !important; word-wrap: break-word !important; list-style: none !important; height: 20px !important; vertical-align: bottom !important; margin-left: 0px'
	},
	{
		'selector': '.left_adv',
		'style': 'display: inline !important; margin: 0 0 1px 10px; width: 339px !important; border-left: 1px dashed #B5BCC0; list-style:none;'
	},
	{
		'selector': '.right_adv',
		'style': 'margin-left: -5px; margin-bottom: 1px; width: 339px !important; list-style:none !important;'
	},
	{
		'selector': '.left_item',
		'style': 'display: inline !important; margin: 0 0 1px 10px; width: 339px !important; border-left: 1px dashed #B5BCC0; '
	},
	{
		'selector': '.right_item',
		'style': 'margin-bottom: 1px; width: 339px !important; height: 18px; '
	},
	{
		'selector': '#rel_list li a',
		'style': 'color: #333; text-decoration: none; margin-bottom: 1px; padding-left: 25px; height: 18px; background: url(' + rel_resource_url + '/dot.png) no-repeat 10px 5px;'
	}
];
...
function RelateSubjectCSS() {
	if (document.all) {
		var style = document.createStyleSheet();
		if (rsCss.length) {
			for (var i=0; i<rsCss.length; i++) {
				style.addRule(rsCss[i].selector, rsCss[i].style);
			}
		}
	} else {
		var cssStyle = '';
		for (var i=0; i<rsCss.length; i++) {
			cssStyle += rsCss[i].selector + '{ '+ rsCss[i].style + '}';
		}
